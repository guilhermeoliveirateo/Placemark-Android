...
fun addPlacemark() {
    ...
    val lat = readlnOrNull()?.trim().orEmpty()
    print("Enter Longitude: ")
    val lng = readlnOrNull()?.trim().orEmpty()

    if (title.isNotEmpty()) {
        val placemark = PlacemarkModel(title = title, description = description, lat = lat, lng = lng)
...
fun listPlacemarks() {
    ...
    if (placemarks.isEmpty()) {
        ...
        placemarks.forEach { println("ID: ${it.id} | Title: ${it.title} | Description: ${it.description} | Latitude: ${it.lat} | Longitude: ${it.lng}") }
    }
}
...
fun updatePlacemark() {
    ...
    if (id != null && store.findOne(id) != null) {
        ...
        val description = readlnOrNull()?.trim().orEmpty()
        print("Enter New Latitude: ")
        val lat = readlnOrNull()?.trim().orEmpty()
        print("Enter New Longitude: ")
        val lng = readlnOrNull()?.trim().orEmpty()

        if (title.isNotEmpty()) {
            val updated = store.update(PlacemarkModel(id = id, title = title, description = description, lat = lat, lng = lng))
...
fun searchPlacemark() {
    ...
    if (id != null) {
        ...
        if (placemark != null) {
            println("Found: ID: ${placemark.id} | Title: ${placemark.title} | Description: ${placemark.description} | Latitude: ${placemark.lat} | Longitude: ${placemark.lng}")
        } else {
